 ...
 *     @author Mechalikh
 **/
package com.pureedgesim.datacentersmanager;
...
import com.pureedgesim.locationmanager.MobilityModel;
...
public class DataCentersManager {
	...
	private void selectOrchestrators() {
		for (DataCenter edgeDataCenter : datacentersList) {
			if ("".equals(SimulationParameters.DEPLOY_ORCHESTRATOR) || ("CLOUD".equals(SimulationParameters.DEPLOY_ORCHESTRATOR) && edgeDataCenter.getType() == SimulationParameters.TYPES.CLOUD)) {
				edgeDataCenter.setAsOrchestrator(true);
				orchestratorsList.add(edgeDataCenter);
			} else if ("EDGE".equals(SimulationParameters.DEPLOY_ORCHESTRATOR) && edgeDataCenter.getType() == SimulationParameters.TYPES.EDGE_DATACENTER) {
				edgeDataCenter.setAsOrchestrator(true);
				orchestratorsList.add(edgeDataCenter);
			} else if ("MIST".equals(SimulationParameters.DEPLOY_ORCHESTRATOR) && edgeDataCenter.getType() == SimulationParameters.TYPES.EDGE_DEVICE) {
				edgeDataCenter.setAsOrchestrator(true);
				orchestratorsList.add(edgeDataCenter);
			} else if ("MIST_EDGE".equals(SimulationParameters.DEPLOY_ORCHESTRATOR) && (edgeDataCenter.getType() == SimulationParameters.TYPES.EDGE_DATACENTER || edgeDataCenter.getType() == SimulationParameters.TYPES.EDGE_DEVICE)) {
				edgeDataCenter.setAsOrchestrator(true);
				orchestratorsList.add(edgeDataCenter);
			} else if ("ALL".equals(SimulationParameters.DEPLOY_ORCHESTRATOR)) {
				edgeDataCenter.setAsOrchestrator(true);
				orchestratorsList.add(edgeDataCenter);
			}
		}
