 ...
 *     @author Mechalikh
 **/
package com.pureedgesim.scenariomanager;
...
public class FilesParser {
	...
	private boolean checkSimulationProperties(String simProp) {
		...
		try {
			...
			SimulationParameters.SIMULATION_TIME = SimulationParameters.INITIALIZATION_TIME	+ (double) 60 * Double.parseDouble(prop.getProperty("simulation_time").trim()); // seconds
			...
			SimulationParameters.WAIT_FOR_TASKS_LIMIT = Integer.parseInt(prop.getProperty("wait_for_all_tasks_limit").trim());
			...
			for (int i = 0; i < datacenterList.getLength(); i++) {
				...
				if (type == TYPES.EDGE_DEVICE) {
					if (!checkEdgeDevice(datacenterElement))
						return false;
					percentage += Double.parseDouble(datacenterElement.getElementsByTagName("percentage").item(0).getTextContent());
	...
	private boolean checkEdgeDevice(Element datacenterElement) {
		...
		double percentage = Double.parseDouble(datacenterElement.getElementsByTagName("percentage").item(0).getTextContent());
	...
	private boolean checkAppFile(String appFile) {
		...
		try {
			...
			for (int i = 0; i < appList.getLength(); i++) {
				...
				double max_delay = Double.parseDouble(appElement.getElementsByTagName("max_delay").item(0).getTextContent());

				// the size of the container (KB)
				long container_size = Long.parseLong(appElement.getElementsByTagName("container_size").item(0).getTextContent());

				// average request size (KB)
				long request_size = Long.parseLong(appElement.getElementsByTagName("request_size").item(0).getTextContent());

				// average downloaded results size (KB)
				long results_size = Long.parseLong(appElement.getElementsByTagName("results_size").item(0).getTextContent());
				...
				int required_cores = Integer.parseInt(appElement.getElementsByTagName("required_core").item(0).getTextContent());
				...
				int usage_percentage = Integer.parseInt(appElement.getElementsByTagName("usage_percentage").item(0).getTextContent());
