...
import com.pureedgesim.network.NetworkModel;
import com.pureedgesim.scenariomanager.Scenario;
...
import com.pureedgesim.simulationvisualizer.SimulationVisualizer;
...
public class SimulationManager extends SimulationManagerAbstract {
	...
	public void processEvent(SimEvent ev) {
		...
		case PRINT_LOG:
			...
			if (SimulationParameters.WAIT_FOR_TASKS && (tasksCount / simLog.getGeneratedTasks()) < tasksPercentageStop) {
				...
				if(SimulationParameters.WAIT_FOR_TASKS_LIMIT == 0 || simulation.clock() < SimulationParameters.WAIT_FOR_TASKS_LIMIT*60) {
	...
	private void sendFromOrchToDestination(Task task) {
		...
		// Stop in case no resource was available for this task, the offloading is failed
	...
	public double getTotalFailuresRateSimLog() {
		return simLog.getTasksSent() == 0.0 ? 0.0 : (simLog.getTasksFailed() * 100.0) / simLog.getTasksSent();
	}
	
	public int getExecutedTasksCount() {
		return simLog.getExecutedTasksCount();
	}

	public int getTasksCount() {
		//return tasksCount;
		return simLog.getTasksSent();
	}
	
	public int getTasksFailedCount() {
		//return failedTasksCountTotal;
		return simLog.getTasksFailed();
	}
	
	public int getTasksFailedDueLatency() {
		return simLog.getTasksFailedLatency();
	}
	
	public int getTasksFailedDueBattery() {
		return simLog.getTasksFailedBeacauseDeviceDead() + simLog.getTasksNotGeneratedBeacuseDeviceDead(); // TODO: Revisar
	}
	
	public int getTasksFailedDueMobility() {
		return simLog.getTasksFailedMobility();
	}
	...
	public double getTotalWaitingTime() {
		return simLog.getTotalWaitingTime();
	}

	public boolean taskFailed(Task task, int phase) {
		task.setCheckTime(task.getSimulation().clock());
